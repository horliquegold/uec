...
#if defined(NDEBUG)
# error "Unitedemiratecoin cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "Unitedemiratecoin Signed Message:\n";
...
{
    // Unitedemiratecoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Unitedemiratecoin: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // Unitedemiratecoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Unitedemiratecoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Unitedemiratecoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("UnitedemiratecoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("UnitedemiratecoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static UnitedemiratecoinMiner(CWallet *pwallet)
{
    printf("UnitedemiratecoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("unitedemiratecoin-miner");
    ...
    try { loop {
        ...
        printf("Running UnitedemiratecoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("UnitedemiratecoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&UnitedemiratecoinMiner, pwallet));
}
